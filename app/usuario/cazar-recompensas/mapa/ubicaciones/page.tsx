...
export default function UbicacionesPage(){
 ...
 <section className={styles.campaignCarousel}>{campaign.locations.map((location,index)=><Link key={location.id} href={`/usuario/cazar-recompensas/mapa/jugar?campaign=${encodeURIComponent(campaign.id)}&location=${encodeURIComponent(location.id)}`} className={styles.campaignCard}><div className={styles.campaignIcon}><MapPin/></div><div className={styles.campaignBody}><span>Ubicación {index+1}</span><h2>{location.name}</h2><p>{location.reward}</p><div className={styles.campaignMeta}><div><Gift/><span>{location.availableUnits} disponible{location.availableUnits===1?"":"s"}</span></div></div></div><ChevronRight/></Link>)}</section>}
 </main>;
}
