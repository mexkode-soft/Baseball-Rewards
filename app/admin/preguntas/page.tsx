...
import { readQuestions, saveQuestions, type QuestionCategory, type TriviaQuestion } from "@/lib/questions";
...
export default function Page() {
  ...
  function persist(next:TriviaQuestion[]){setQuestions(next);saveQuestions(next);}
  function save(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!text.trim()||answers.some(a=>!a.trim()))return;
    persist([{id:Date.now(),text:text.trim(),category,brand:category==="marca"?brand.trim():undefined,answers:[answers[0],answers[1],answers[2]],correctAnswer},...questions]);
    setText("");setBrand("");setAnswers(["","",""]);setCorrectAnswer(0);
  }
  ...
  <section className={styles.card}><div className={styles.listHeading}><span>Banco de preguntas</span><h2>{questions.length} preguntas</h2></div><div className={styles.list}>{questions.map(q=><article key={q.id}><div className={styles.questionTop}><div><span>{q.category}</span>{q.brand&&<b>{q.brand}</b>}</div><button type="button" onClick={()=>persist(questions.filter(i=>i.id!==q.id))}><Trash2/></button></div><h3>{q.text}</h3><div className={styles.answers}>{q.answers.map((answer,index)=><div key={answer} className={index===q.correctAnswer?styles.correct:""}>{index===q.correctAnswer&&<CheckCircle2/>}{answer}</div>)}</div></article>)}</div></section></div></>;
}
