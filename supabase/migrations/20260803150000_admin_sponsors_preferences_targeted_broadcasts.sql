-- Planes, preferencias globales, alta controlada de patrocinadores
-- y audiencias dirigidas.
--
-- Corrección principal:
-- public.app_settings ya existe con las columnas:
--   key text primary key
--   value jsonb not null
--   updated_by uuid null
--   updated_at timestamptz not null
--
-- Esta migración usa esa estructura real y es idempotente.

create table if not exists public.subscription_plans (
  code text primary key
    check (code in ('basic', 'intermediate', 'premium')),
  name text not null,
...
do update set
  name = excluded.name,
  allows_ticket = excluded.allows_ticket,
  allows_qr = excluded.allows_qr,
  allows_map = excluded.allows_map,
  max_active_campaigns = excluded.max_active_campaigns;

alter table public.sponsor_organizations
...
where slug = 'marca-demo-home-run';

-- Compatibilidad con la tabla app_settings existente.
-- No se crean setting_key/setting_value porque la tabla real usa key/value.
...
drop policy if exists app_settings_read
...
drop policy if exists app_settings_admin
  on public.app_settings;
...
drop policy if exists sponsor_org_admin_write
...
drop policy if exists sponsor_members_admin_write
  on public.sponsor_members;

create policy sponsor_members_admin_write
  on public.sponsor_members
  for all
  using (public.is_admin())
  with check (public.is_admin());
...
declare
  ...
  v_amount integer := greatest(
    1,
    coalesce(
      nullif(p_audience ->> 'amount', '')::integer,
      1
    )
  );
  v_ids uuid[] := array(
...
begin
  ...
  if v_audience_type = 'specific'
     and coalesce(array_length(v_ids, 1), 0) = 0 then
  ...
  with eligible as (
    ...
    where (
      ...
      or (
        v_audience_type = 'location'
        and p.role = 'usuario'
        and p.state = v_state
      )
      or (
        v_audience_type = 'level'
        and p.role = 'usuario'
        and exists (
          select 1
      ...
      or (
        v_audience_type = 'random'
        and p.role = 'usuario'
      )
    )
    order by
      case
        when v_audience_type = 'random' then random()
        else 0
      end,
      p.id
    limit case
      when v_audience_type = 'random' then v_amount
      else 2147483647
    end
  ),
  inserted as (
    ...
    select
      ...
      p_message_type,
      case
        when p_action_url = '/usuario'
             and v_audience_type = 'sponsors'
          then '/patrocinador'
        else p_action_url
      end,
      p_image_url
...
grant execute
on function public.publish_broadcast(
  text,
  text,
  text,
  text,
